...
import { sponsorshipApi } from '@/lib/sponsorship-api'
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
interface DealDetailsModalProps {
  ...
  onStatusChange?: (updatedDeal: Sponsorship) => void
}
...
export default function DealDetailsModal({
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    setError(null)
    try {
      const updated = await sponsorshipApi.updateSponsorship(deal.id, { 
        status: newStatus as any 
      })
      onStatusChange?.(updated)
      console.log('[DealDetailsModal] Status updated successfully:', newStatus)
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to update status'
      setError(message)
      console.error('[DealDetailsModal] Failed to update status:', err)
    } finally {
